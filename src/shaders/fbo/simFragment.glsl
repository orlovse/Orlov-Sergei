...
void main(){
  ...
  float circularForce = 1.0 - smoothstep(0.5, 1., abs(pos.x - radius));
  ...
  radius += (targetRadius - radius) * mix(0.2, 1., circularForce);

  vec3 targetPos = vec3(cos(angle), sin(angle), 0.0) * radius;

  pos.xy += (targetPos.xy - pos.xy) * 0.1;

  pos.xy += curl(pos.xyz * 2., time * 0.1, 0.1).xy * 0.006;
